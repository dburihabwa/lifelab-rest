...
class InitCommand extends ContainerAwareCommand {
	...
	protected function configure() {
		$this->setName('lifelab:init')
			->setDescription('Setup the database with dummy data!');
	}

	protected function execute(InputInterface $input, OutputInterface $output) {
		$this->em = $this->getContainer()->get('doctrine')->getManager();
		echo("Setting up dummy medical file... ");
    ...
    public function contains($prose, $pattern){
       return strpos(strtolower($prose), utf8_decode(strtolower($pattern))) !== false;
    }

    public function addMedicine($medic){
